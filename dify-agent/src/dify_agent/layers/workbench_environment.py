...
class WorkbenchEnvironmentLayer(PydanticAILayer[NoLayerDeps, object, EmptyLayerConfig, EmptyRuntimeState]):
    ...
    def prefix_prompts(self):
        return ["Use update_shared_environment to install or upgrade Python or Node registry packages. "
    ...
    def _prepare(self, _ctx: RunContext[object], _definition: ToolDefinition):
        return ToolDefinition(name=TOOL_NAME, description="Install or upgrade shared Python/Node dependencies atomically.",
            parameters_json_schema=EnvironmentArgs.model_json_schema(), sequential=True, kind="external")
    ...
    def build_deferred_tool_call_payload(self, requests: DeferredToolRequests):
        ...
        args = EnvironmentArgs.model_validate_json(call.args) if isinstance(call.args, str) else EnvironmentArgs.model_validate(call.args)
        return DeferredToolCallPayload(tool_call_id=call.tool_call_id, tool_name=call.tool_name,
