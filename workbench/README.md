...
个人 MCP 的配置保存在用户沙盒的 `mcp/<id>/mcp.json`，支持远程 HTTP/SSE 和本地 stdio。配置、工具发现、账号隔离及镜像更新步骤见 [个人 MCP](personal-mcp.md)。
...
普通执行时工具和 Skills 默认保持可用。计划模式仅开放内置调查、澄清和计划审阅工具；附件解析与临时渲染通过只读、离线的 `plan_inspect` 执行，外部工具和工作区写入须等计划明确批准后进行。点名要求只属于发送时的那一轮：Skills 在运行开始前读取，每个点名工具组须至少执行一个适用工具后才能给出最终答复；参数校验失败不算执行。询问用户和计划审阅可以先进行，同一任务暂停恢复会保留已执行状态。工具返回的失败结果仍须如实说明。

计划校验覆盖普通工具和延迟执行工具；共享环境更新在 Agent、API 接收和安装派发阶段分别检查。最终正文通过计划与文件检查后才推送给页面。调查停止会使旧执行许可失效，并清理初始化中和运行中的调查容器。

此次模式版本按整体切换交付：API、Agent、Sandbox Manager 及前端配套升级，使用新建对话。部署方清理旧对话后切换；新增控制状态协议不提供旧版本混用或直接回滚兼容。
