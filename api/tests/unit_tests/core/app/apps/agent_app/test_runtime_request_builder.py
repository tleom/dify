...
def test_workbench_rebuild_accepts_previous_composition(previous_prompt: str, current_prompt: str):
    ...
    previous = builder.build(replace(_ctx(previous_soul), workbench_run_id="run-1")).request
    snapshot = _snapshot_for_layer_names([layer.name for layer in previous.composition.layers])
    current_soul = _soul_with_model()
    current_soul.prompt.system_prompt = current_prompt
    result = builder.build(replace(_ctx(current_soul, session_snapshot=snapshot), workbench_run_id="run-2"))
    assert result.request.rebuild_layers is True
