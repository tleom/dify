...
def setup(monkeypatch, **overrides):
    run = SimpleNamespace(
        id="workbench",
        tenant_id="tenant",
        account_id="account",
    ...
    monkeypatch.setattr(context_status.dify_config, "WORKBENCH_ENABLED", True)
    redis = Mock()
...
def test_persists_latest_reading_and_merges_compaction_in_stream_order(monkeypatch):
...
def test_ignores_wrong_backend_attempt_and_other_account(monkeypatch):
