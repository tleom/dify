...
@pytest.fixture
def pg_queue(monkeypatch, config_overrides, tmp_path):
    ...
    fixture = test_followups.queue.__wrapped__(monkeypatch, config_overrides, tmp_path)
    queue = next(fixture)
...
def test_context_lock_blocks_concurrent_payload_writer_without_losing_fields(pg_queue, monkeypatch, change):
    queue = pg_queue
    ...
    def hold_row(*args, **kwargs):
        run = current_run(*args, **kwargs)
        entered.set()
...
def test_paused_queue_continuation_on_postgresql(pg_queue, query):
    test_followups.test_paused_queue_waits_and_manual_continuation_precedes_all_three_messages(pg_queue, query)


def test_recovery_ignores_historical_pause_and_keeps_bounded_batches_on_postgresql(pg_queue):
...
def test_attachment_only_with_knowledge_on_postgresql(pg_queue, monkeypatch, phase):
...
def test_continue_uses_original_revision_on_postgresql(pg_queue, monkeypatch):
...
def test_automatic_recovery_keeps_queue_order_on_postgresql(pg_queue, monkeypatch, ending, queue_after_failure):
    test_followup_recovery.local_control.__wrapped__(monkeypatch)
...
def test_pause_and_continue_during_recovery_on_postgresql(pg_queue, monkeypatch, create_successor):
    test_followup_recovery.local_control.__wrapped__(monkeypatch)
    test_followup_recovery.test_pause_cancels_recovery_and_keeps_queue_until_blank_continue(pg_queue, create_successor)


def test_steering_during_recovery_on_postgresql(pg_queue, monkeypatch):
    test_followup_recovery.local_control.__wrapped__(monkeypatch)
...
@pytest.mark.parametrize("ending", ["failed", "interrupted"])
def test_newer_ancestor_does_not_supersede_waiting_task_on_postgresql(pg_queue, monkeypatch, continuation, ending):
    test_followup_recovery.local_control.__wrapped__(monkeypatch)
...
def test_cleanup_proof_tracks_the_current_ticket_on_postgresql(pg_queue, ending):
    ...
    with pg_queue.factory.begin() as session:
        ...
        session.get(WorkbenchRun, item["id"]).backend_run_id = str(uuid4())
    followups.save_fenced_state(ticket, {"history": None})
    with pg_queue.factory() as session:
        assert session.scalar(select(WorkbenchRun.id).where(recovery.cleanup_pending_condition())) == item["id"]
