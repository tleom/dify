...
def test_chinese_skill_labels_keep_published_execution_ids_and_scope(monkeypatch):
...
def test_empty_skill_catalog_does_not_query_database(monkeypatch):
...
def test_saved_badge_is_chinese_while_model_resource_token_uses_skill_identifier():
